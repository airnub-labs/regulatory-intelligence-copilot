...
SELECT
    ...
    '✓' AS status
FROM auth.users u
LEFT JOIN copilot_core.user_preferences up ON up.user_id = u.id
LEFT JOIN copilot_core.tenants t ON t.id = up.current_tenant_id
LEFT JOIN copilot_core.tenant_memberships tm ON tm.user_id = u.id AND tm.tenant_id = t.id
