...
> **Status:** v0.4  
> **Supersedes:** `regulatory_graph_copilot_concept_v_0_3.md`  
> **Companion specs:**
...
The graph model (see `graph_schema_v_0_4.md`) expresses this as:
...
| **LLM Router + providers**       | Provider‑agnostic LLM layer (OpenAI Responses incl. GPT‑OSS, Groq, local/OSS models), behind Egress Guard     |
...
| **Next.js 16 + Vercel AI SDK v5**| Web app shell: chat UI, SSE streaming, calling `/v1/chat` and graph endpoints, not tied to any single provider|
...
- Backed by `graph_schema_v_0_4.md` for structure.
...
- Expressed in `graph_schema_v_0_4.md` using explicit edge types for different time aspects.
