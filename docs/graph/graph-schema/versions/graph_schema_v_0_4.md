...
- Optional graph algorithms (see `graph_algorithms_v_0_1.md`).
...
7. **Algorithm‑agnostic but algorithm‑friendly**  
   ...
   - May add **derived metadata** (properties, helper nodes) as described in `graph_algorithms_v_0_1.md`.
...
> `docs/safety/safety-guards/graph_ingress_guard_v_0_1.md`, which is applied by the
...
Graph algorithms described in `graph_algorithms_v_0_1.md` may introduce **derived metadata** to support GraphRAG and explanation, for example:
...
- Changes from v0.2:
  ...
  - Recognised algorithm‑derived metadata as optional and governed by `graph_algorithms_v_0_1.md` and the Graph Ingress Guard.
- Further changes should be recorded in the appropriate `graph_schema_changelog_v_*.md` file with migration notes where needed.
