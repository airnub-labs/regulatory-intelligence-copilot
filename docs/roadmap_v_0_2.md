# ROADMAP – Regulatory Intelligence Copilot (v0.2)
...
>
> This roadmap reflects the v0.2 architecture: jurisdiction‑neutral prompts, prompt aspects, provider‑agnostic LLM routing (including OpenAI Responses + GPT‑OSS + local/OSS models), direct Memgraph GraphClient, WebSocket graph streaming, and a reusable engine + demo app split.
...
**Goal:** Stand up the new repo with a clean foundation that reuses working infra from `rfc-refactor` but drops HTTP/RFC auditing.

**Tasks**
- [ ] Confirm fork: `rfc-refactor` → `regulatory-intelligence-copilot`.
- [ ] Replace legacy docs with new ones:
  - [ ] `README.md` → regulatory copilot positioning.
  - [ ] `ARCHITECTURE_v0_2.md`.
  - [ ] `DECISIONS_v0_2.md`.
  - [ ] `AGENTS.md` (new agents + orchestrator).
  - [ ] `ROADMAP_v0_2.md` (this document).
...
- [ ] Ensure dev environment runs end-to-end with stub logic:
  - [ ] `pnpm dev` works.
  - [ ] `/api/chat` returns a trivial stubbed answer from the new Compliance Engine surface.

**Exit criteria**
- New repo builds and runs cleanly.
...
**Goal:** Implement the core **engine primitives** and **LLM routing** so all future work builds on the right abstractions.

**Scope**
- Introduce engine packages.
- Implement provider‑agnostic LLM layer (including OpenAI Responses + GPT‑OSS, Groq, and local/OSS models).
...
- [ ] Repo structure:
  ...
  - [ ] Create `packages/reg-intel-llm` – `LlmClient`, `LlmRouter`, backends, egress guard integration.
...
- [ ] LLM router & providers:
  ...
  - [ ] Implement `OpenAiResponsesClient` using **Responses API** (including GPT‑OSS models), no legacy chat completions.
  - [ ] Implement `GroqLlmClient`.
  - [ ] Implement `LocalHttpLlmClient` for talking to local/OSS model servers (vLLM/Ollama etc.).
  - [ ] Define `TenantLlmPolicy` and basic config store for per-tenant/per-task policies.
- [ ] Egress guard:
...
**Goal:** Move the graph and timeline responsibilities into a clean abstraction and seed a minimal but realistic regulatory graph.

**Scope**
- Implement `GraphClient` (direct Memgraph access).
- Implement a basic `TimelineEngine`.
- Seed a small graph for a single-director Irish persona.
...
- [ ] Seed a tiny but realistic initial graph for Ireland:
  ...
  - [ ] Basic edges: `APPLIES_TO`, `REQUIRES`, `EXCLUDES`, `LOOKBACK_WINDOW`, `LOCKS_IN_FOR_PERIOD`.
  - [ ] Initial profile tags: `PROFILE_SINGLE_DIRECTOR_IE` etc., generated via new dynamic profile tag logic.

**Exit criteria**
- A minimal Compliance Engine instance can:
  - Query Memgraph via `GraphClient`.
  - Use `TimelineEngine` to reason about at least one lookback window.
- You can ask a very small question about a single-director IE scenario and get an answer that references real nodes from Memgraph.
...
- [ ] Graph endpoints:
  ...
  - [ ] Backend patch generation strategy (e.g. diff against last known snapshot, or event-based updates from ingestion jobs).
- [ ] Graph UI:
...
- The demo app presents:
  - A working chat that uses the v0.2 engine.
...
**Goal:** Deliver a genuinely useful Ireland-first vertical slice with multiple domain agents and non-trivial interactions.
...
- [ ] On-demand enrichment:
  ...
  - [ ] Ensure the graph is incrementally updated during real conversations (honouring D-010).
- [ ] Change-tracking:
