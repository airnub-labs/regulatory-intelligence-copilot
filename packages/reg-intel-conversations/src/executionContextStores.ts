...
/**
 * In-memory implementation of ExecutionContextStore
...
export class InMemoryExecutionContextStore implements ExecutionContextStore {
  ...
  async touchContext(contextId: string, ttlMinutes: number = 30): Promise<void> {
    ...
    this.logger?.info('[InMemoryExecutionContextStore] Touched context', {
      contextId,
  ...
  async updateStatus(
    ...
    this.logger?.info('[InMemoryExecutionContextStore] Updated status', {
      contextId,
      status,
      errorMessage,
    });
  ...
  async terminateContext(contextId: string): Promise<void> {
    ...
    this.logger?.info('[InMemoryExecutionContextStore] Terminated context', {
      contextId,
