...
export const ALLOWED_NODE_LABELS = [
  'Jurisdiction',
  'Region',
  'Concept',
  'Label',
...
export const schemaValidationAspect: GraphIngressAspect = async (ctx, next) => {
  if (ctx.nodeLabel && !ALLOWED_NODE_LABELS.includes(ctx.nodeLabel)) {
    throw new Error(
      `Graph Ingress Guard: Disallowed node label "${ctx.nodeLabel}". ` +
        `Allowed labels: ${ALLOWED_NODE_LABELS.join(', ')}`,
    );
  }

  if (ctx.relType && !ALLOWED_RELATIONSHIP_TYPES.includes(ctx.relType)) {
    throw new Error(
      `Graph Ingress Guard: Disallowed relationship type "${ctx.relType}". ` +
        `Allowed types: ${ALLOWED_RELATIONSHIP_TYPES.join(', ')}`,
    );
  }
