...
export async function GET(
  ...
  try {
    ...
    // Get all tenant memberships (including removed ones for restore functionality)
    ...
    // Map memberships with tenant info and split into active/removed
    ...
    const removedMemberships: typeof activeMemberships = []
    ...
    for (const m of memberships || []) {
      ...
      if (m.deleted_at) {
        removedMemberships.push(membership)
      } else {
    ...
    // Only show team/enterprise tenants (not personal tenants which belong to specific users)
    const { data: allTenants } = await supabase
      .schema("copilot_core")
      .from("tenants")
      .select("id, name, slug, type, plan")
      .is("deleted_at", null)
      .neq("type", "personal") // Personal tenants can only be owned by their creator
      .order("name")

    // Filter out tenants user is already in (active memberships only)
    const activeTenantIds = activeMemberships.map((m) => m.tenantId)
    const availableTenants = (allTenants || []).filter(
      (t) => !activeTenantIds.includes(t.id)
    )
